...
const updateAgent = db.prepare(`
  UPDATE agents SET status = @status, session_id = @sessionId, session_title = @sessionTitle, working_since = @workingSince WHERE id = @id
`);
...
export const storage = {
  ...
  updateAgent(id: string, updates: { status?: string; sessionId?: string; sessionTitle?: string; workingSince?: number | null }) {
  ...
  // Xoá hội thoại MAIN: msg do orchestrator tạo, msg gửi tới orchestrator (user/agent), msg user gửi main.
  // Giữ hội thoại riêng của agents (user↔agent, agent→user).
